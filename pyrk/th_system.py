...
class THSystem(object):
        ...
        if isinstance(component, THSuperComponent):
            ...
            for interface, d in six.iteritems(component.cond):
                ...
                if component.sph:
                    ...
                    Qcond = self.conduction_slab(
                        component, env, t_idx, L=d["L"], A=d["area"]
                    )
                to_ret -= Qcond / cap
            for interface, d in six.iteritems(component.conv):
                ...
                if isinstance(env, THSuperComponent):
                    Tr = env.compute_tr(
                        ...
                        h=d["h"].h(component.rho(t_idx), component.mat.mu).magnitude,
                    )
                    Qconv = self.convection(
                        t_b=component.T[t_idx].magnitude,
                        t_env=Tr,
                        h=d["h"].h(component.rho(t_idx), component.mat.mu),
                        A=d["area"],
                    )
                    assert (
                        Qconv * (component.T[t_idx].magnitude - Tr)
                    ) >= 0, """
                    ...
                    if isinstance(component.mat, LiquidMaterial):
                        h_conv = d["h"].h(component.rho(t_idx), component.mat.mu)
                    else:
                    ...
                    assert (
                        Qconv * (component.T[t_idx] - env.T[t_idx])
                    ).magnitude >= 0, (
    ...
    def heatgen(self, component, power, omegas):
        ...
        """
        return (
            power * component.power_tot.magnitude * (1 - self.kappa) + sum(omegas)
        ) / component.vol.magnitude
