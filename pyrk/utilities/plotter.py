...
def plot_reactivity(x, si):
    ...
    :type x: np.ndarray"""
    plt.plot(x, si.ne._rho, color=my_colors(1, len(si.ne._rho)), marker=".")
    plt.xlabel("Time [s]")
...
def plot_power_w_reactivity(x, si, y):
    ...
    plt.plot(x, rho, color=my_colors(1, 2), marker=".", label="External Reactivity")
    plt.legend()
    plt.xlabel("Time [s]")
...
def plot_temps_together(x, y, si):
    for num, comp in enumerate(si.components):
        ...
        plt.plot(
            ...
            y[:, idx],
            label=comp.name,
            color=my_colors(num, len(si.components)),
            marker=".",
        )
    plt.legend()
    plt.xlabel("Time [s]")
...
def plot_zetas(x, y, si):
    for num in range(0, si.ne._npg):
        ...
        plt.plot(
            x,
            y[:, idx],
            color=my_colors(num, si.ne._npg),
            marker=".",
            label="i = " + str(idx),
        )
    plt.xlabel(r"Time $[s]$")
...
def plot_omegas(x, y, si):
    for num in range(0, si.ne._ndg):
        ...
        plt.plot(
            x,
            y[:, idx],
            color=my_colors(num, si.ne._ndg),
            marker=".",
            label="i = " + str(idx),
        )
    plt.legend()
